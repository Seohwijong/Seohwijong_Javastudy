 ...
 *            = 다른 데이터형 : 클래서(******)
 *      3. 클래스
 *         = 클래스 정의 방법 (객체지향 프로그램 => 한개의 클래스로 만드는 것이 아닌 
 *                        클래스와 클래스끼리의 연관관계 => Spring(DI))
 *                        행위자(웹: 사용자 , 관리자)
 *           *** 접근지정어 (권한)
 ...
 *              class B extends A
 *              {
 *                  void display(){} ==> 오류발생(축소가 불가능하기때문에 public => default로의 변환은 불가)
 *              }
 *              class A
 *              {
 *                  void display(){}
 ...
 *              class B extends A
 *              {
 *                  public void display(){} ==> 확장이 가능하기때문에 default => public로의 변환은 가능
 *              }
 *              상속 : 반복제거
 *              class A
 *              {
 ...
 *              	 	this 사용이 가능
 *              	 }
 *               	static void aaa() ==> 소송이 아니고 공유하는 메소드
 *               	{
 *               		this가 존재하지 않는다
...
// Super로부터 Sub가 상속을 받았다 => 예외조건) 생성자 , 초기화블록 , static (공유)
// private => 상속은 가능 => 접근이 불가능
// ------- protected를 사용하는게 접근성이 더 좋음
// ------- getter / setter 
class Sub extends Super
{
