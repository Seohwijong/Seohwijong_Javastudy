 ...
 *         finally
 *         {
 *            생략이 가능 : 무조건 수행한다 (try ~ catch 상관없다)
 *            => 서버 닫기 , 오라클 닫기 , 파일 닫기 ...
 *         }
 ...
 *         *** try절에서 에러 발생시에는 에러가 발생한 곳 부터는 수행하지 않는다
 ...
 *             --------
 *         }
 *         ==> catch는 위에서 한개씩 검색한 후에 해당 예외처리 클래스를 찾아 준다
 *                    ------------------ 다중 조건문과 동일한 방식 (catch는 한개만 수행)
 *         = 간접처리 (예외회피) : 자바나 오픈 라이브러리
 *           예외에 대해 예측되는 예외클래스 선언만 한다
