 ...
 *      String s1="Hello ";
 *      String s2="Java";
 *      s1.concat(s2) ==> "Hell Java";
 *      
 *      ===> concat ==> MySQL LIKE문장을 만들때
 *           WHERE name LIKE CONCAT('%' , '흥' , '%')
 *           WHERE name LIKE '%'|흥|'%'
 ...
 *      -------- ID , PWD       ------------------ 영어검색 , 영문이름
 *      indexOf() , lastIndexOf() --- 뒤에서 부터 찾는다
 *      --------- 위치확인 (문자 / 문자열)
 *      --------- 앞에서부터 찾기
 *      length() : 문자의 갯수
...
//	}
		String[] data= {"JAVA","java","Java","jAva","JAva","JAVa","jaVA","javA"};
		for(String s:data)
		{
			if(s.equals("java"))
			{
		...
		System.out.println("====================");
		System.out.println("가장 긴 이름의 과일명 출력");
		int max=0;
		for(String s:data2)
		{
			if(max<s.length())
...
//				}catch(Exception ex) {}
		...
		String[] color=ss.split("\\?"); // | 정규식에서 사용하는 기호
		// | , ? , . , + , * , ^
		for(String s:color)
		{
			System.out.println(s);
