 ...
 *               ------
 *               
 *               Object : 모든 클래스의 상위 클래스 (모든 클래스는 Object 상속을 가지고 있다) => 최상위 클래스
 *                 |
 ...
 *              => 스프링에서 주로 상ㅇ하는 기술 (리플렉션)
 *              						 ------ private 까지 접근이 가능
 *           A a=new A(); Class.forName("A")
 *           
 *           => InterriptedException : 쓰레드 (한개의 프로세스 안에서 여러개의 프로그램을 동시에 사용)
 *              충돌과 관련 
 *           => java.lang /java.util => 예외처리와 관련 없다 
 *           => 예외처리는 필수 : 정상 상태를 유지 => 견고한 프로그램
 *     2) UnCheckException : 예외처리가 필요한 경우에만 사용
 *               Exception
 *                   |
 *               RuntimeException : 실행시 (인터프리터) => 사용자 입력 , 프로그램 실수
 *                   |
 *               NumberFormatExcetion : 정수 변환에서 에러
 ...
 *                     Jsoup : 데이터 수집 (String)
 *               ArrayIndeOutOfBoundsException : 배열 범위 초과
 *               ----------------------------- 컬랙션
