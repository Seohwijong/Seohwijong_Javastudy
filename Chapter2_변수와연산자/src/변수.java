/*
 * 	1. 기본형 : 메모리 크기를 지원한다
 * 				1byte / 2byte / 4byte / 8byte => 자바에서 지원
 * 				=> 12byte 저장하려면 두세개의 메모리를 모아서 사용 (2+2+8)   
 * 			    4+4+4 	= 배열
 * 				2+4+8+1 = 클래스
 * 
 *
 ...
 *     ㅣ      ----------  	핸드폰:Mobile
 *     ㅣ		   ㅣ	   	윈도창:Application
 *     ㅣ 		  메소드	   	브라우저:WebApplication
 *     ㅣ
 * 	한개의 데이터를 저장			-> 변수			메모리에 저장되서
 * 	같은 데이터 여러개 저장		-> 배열			데이터가 다 사라짐
 * 	다른 데이터 여러개 저장		-> 클래스			렘에 저장하기 때문
 * 
 *  메모리 영구 파일/RDBMS
 * 
 */
 ...
 * 		------------
 * 		1 특징
 * 			= 운영체제의 독립적 (모든 운영체제에 호환성이 좋다)
 * 			  예) 윈도우에서 작성 => 리눅스에서 사용이 가능
 * 			  C/C++ => 소스 작성 ==> 컴파일 (컴퓨터가 인식하는 언어로 변경)
 * 								   .obj
 * 								   운영체제마다 다르다		  (다른 운영체제에서 사용하기위해선 컴파일을 다시해야함)
 * 			  Java  => 소스 작성 ==> 컴파일
 * 								   .class 
 * 								   운영체제마다 동일한 파일 생성 (다른 운영체제에서 사용하기위해서 컴파일을 다시안해도 됨)
 * 			  ** 자바 실행
 * 			  A.java   ==========>      A.class       ==========>   실행 화면 출력
 * 				    	컴파일(javac)     바이트(2진파일)   인터프린터(java)
 * 						두번의 검증을 거친다 (보안이 뛰어나다)
 * 			= 객체 지향 프로그램 (재사용이 편리한 상태) => 유지보수 (대규모 프로그램)
 * 			  재사용 : 수정 , 추가 , 데이터 보호 가 뛰어나야함 ==> 5장에서
 * 					------------ ---------
 * 				  오버라이딩/오버로딩   캡슐화
 ...
 * 				=> 자바언어 => 강력한 API를 가지고있다 (필요한 기능이 이미 제작)
 * 				   => 조립식 (자바 공부 => 직접 구현)
 * 			= 자동 메모리 관리 (가비지 컬렉션)
 * 			= 네트워크 / 분산 / 멀티쓰레드 기능을 가지고 있다
 * 		2. 소스 코딩 방법
 * 			주석 : // => 번역이 안된다 (프로그래머만 보는 상태)
 * 			프로그램은 명령어(JVM) => ; (명령마다 끝에 없으면 오류)
 * 			대소문자를 구분한다	a/A
 * 			{}를 사용
 * 			들여쓰기
 * 			=== 구성요소
 ...
 *				   => 빨간색으로 표시
 *				5) 길이는 제한이 없다 => (3~7 정도가 적당)
 *				   변수 => 의미를 약간 부여
 * 				   a => id , name , address , phone...
 * 			==============================================
 * 			과일 가게에는 사과가 5 , 배가 10 , 바나나 7 , 딸기 1    => 변수는 4개 
 * 		
 ...
 * 			기본형
 * 			-----
 * 			정수 저장 방법					메모리는 4byte가 기본단위 => int가 default인 이유
 ...
 * 				10.5F 10.5D(D는 생략가능 , F가 붙으면 float임)
 * 			문자 저장 방법
 * 				2byte => char
 *					=> 1byte(싱글바이트 => ASC) / 2byte(멀티바이트 => UniCode)
 *					*** 한글이나 중국어 , 독일어 , 일본어.... 한글자당 바이트가 2byte
 *					byte  => bit(8개)  => 256문자
 *					2byte => bit(16개) => 65535문자 사용이 가능
 *					UTF-8
 *					각문자마다 번호 
 * 			논리 저장 방법
...
*/
		...
		//*********
		int int_value=2140000000; // -21.4억~21.4억
		//*********
		long long_value=9999999999999l; // 2의63승 숫자뒤에 L써야함 
		//127을 xxx_value라는 공간에 첨부해라(127을 먼저 읽기떄문에  int로 된 127을 long으로 변경해줘야함)
		// int/long의 구분은 l(L)
		char char_value='홍';
		...
		// 모든 실수는 double인식
		// float/double의 구분은 f(F),d(D) => d(D)는 생략가능
		//*********
		boolean boolean_value=true;
		// boolean은 true/false만
		// 모든 정수는 int
		// 모든 실수는 double
		...
		// 상수 => 데이터형 변수명=값 final int b=20; (final이 붙어있으면 상수)
		int kor=80;
		kor=90;
		kor=100;
		
		final int SCORE=70; // 상수의 변수명은 전부 대문자로 적는다
		// = 대입 , == 같다 != 같지않다 (JaveaScript) => = , === , !==
		// 오라클 (=)
		/*
		 * 		변수
		 * 		선언
		 ...
		 * 			int a=10:
		 * 
		 * 		여러개를 한번에 선언할때 (단 데이터형이 같아야함 전부 int, 전부 double)
		 * 		=>  int a;
		 * 			int b;
		 * 			int c;
		 * 			==> int a,b,c;
